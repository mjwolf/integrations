...
- **Security Auditing and Compliance:** Monitor every request made to Vault to ensure compliance with internal security policies and external regulations like SOC2, PCI-DSS, or HIPAA. The integration tracks who accessed what secret and from where, providing a detailed audit trail.
- **Operational Health Monitoring:** Track the health and performance of the Vault cluster by monitoring operational logs for system errors, leadership changes, and storage backend issues. This allows administrators to proactively address issues before they impact availability.
- **Performance Optimization and Capacity Planning:** Utilize Prometheus-formatted metrics to identify latency in secret retrieval, track active client counts, and monitor resource utilization. This data is essential for scaling the cluster effectively to meet demand.
...
For the Elastic Stack, the following versions are required:
- **Kibana:** version `^8.12.0` or `^9.0.0` is required to support the data stream mappings and dashboards provided by this package.
- **Elastic Agent:** version matching the Kibana requirements.
...
2. Set the `log_format` to `json` within the main configuration body:
   ...
   log_format = "json"
   ```
3. Update your service manager (e.g., systemd) to redirect stdout to a file. In the `[Service]` section of your systemd unit file:
   `StandardOutput=file:/var/log/vault/operational.log`
4. Reload the daemon and restart Vault:
...
3. Generate a periodic token with a policy that allows `read` permissions for the `sys/metrics` path.
...
- **Period** (`period`): Specify how often the Agent should poll the metrics API. Default is `30s`.
...
- **Hashed Secrets in Logs**: By default, Vault hashes secret values in audit logs using HMAC-SHA256. If you cannot see raw secrets, this is expected behavior. Use the `/sys/audit-hash` API to verify values against the hashes in the logs.
